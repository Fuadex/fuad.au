...
for (const line of lines) {
  ...
  if (album) { const ak = artist + "\x00" + album; const sp = albumSpan.get(ak); if (!sp) albumSpan.set(ak, [y, y]); else { if (y < sp[0]) sp[0] = y; if (y > sp[1]) sp[1] = y; } }
...
// Powers the header search (songs/albums by title) AND album pages. Artist names are interned.
//   albums = [title, artistIdx, plays, firstYear, lastYear]
//   tracks = [title, artistIdx, plays, albumIdx]   (albumIdx → albums[], or -1 if unknown)
...
const mediaAlbums = _albumEntries.map(([key, plays], i) => {
  ...
  return [title, _ai(artist), plays, sp ? sp[0] : 0, sp ? sp[1] : 0];
});
const mediaTracks = [...trackPlays.entries()].sort((a, b) => b[1] - a[1]).map(([key, plays]) => {
  ...
  return [title, _ai(artist), plays, albumIdx];
}).filter(Boolean);
const mediaOut = `// GENERATED by build-data.js — lazy media index (header search + album pages).
// albums = [title, artistIdx, plays, firstYear, lastYear]; tracks = [title, artistIdx, plays, albumIdx → albums[]].
