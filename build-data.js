...
// Lets the header search find songs/albums by title, not just artists. Artist names are interned
// (tracks/albums reference them by index) to keep the file small. Rows: [title, artistIdx, plays].
...
const _mediaRows = (map) => [...map.entries()].sort((a, b) => b[1] - a[1]).map(([key, plays]) => {
  const ix = key.indexOf("\x00"); return [key.slice(ix + 1), _ai(key.slice(0, ix)), plays];
}).filter(r => r[0]); // drop blank titles
const mediaTracks = _mediaRows(trackPlays), mediaAlbums = _mediaRows(albumPlays);
const mediaOut = `// GENERATED by build-data.js — lazy media search index for the header search.
// tracks/albums rows = [title, artistIndex, plays]; resolve the artist via artists[artistIndex].
