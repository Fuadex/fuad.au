...
// ─────────── CALENDAR — daily play counts + each day's top artist (lazy file) ───────────
{
  const yset = new Set();
  ...
  fs.writeFileSync(path.join(__dirname, "calendar.js"), "// GENERATED by build-data.js — daily listening calendar (lazy-loaded)\nwindow.ROTATION_CAL = " + JSON.stringify(CAL) + ";\n", "utf8");
  console.log(`calendar.js written (${(fs.statSync(path.join(__dirname, "calendar.js")).size / 1024).toFixed(0)} KB · ${calYears.length} years · peak day ${maxDay})`);
}
