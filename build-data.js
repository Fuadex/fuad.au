...
const albMood = new Map();   // albumIdx → { e, v, w } play-weighted energy/valence accumulator
const mediaTracks = [...trackPlays.entries()].sort((a, b) => b[1] - a[1]).map(([key, plays]) => {
  ...
  if (td && td.length >= 10 && albumIdx >= 0) {
    let a = albMood.get(albumIdx); if (!a) albMood.set(albumIdx, a = { e: 0, v: 0, w: 0 });
    a.e += td[4] * plays; a.v += td[5] * plays; a.w += plays;
  }
  const row = [title, _ai(artist), plays, albumIdx, _tail(trackYear.get(key))];
  if (td && td[3]) row.push(td[3]);   // [5] = Spotify track number (sparse; only when known)
  return row;
}).filter(Boolean);
// play-weighted mean mood per album → media album row [8] = [energy, valence] (0..100)
for (const [ai, a] of albMood) if (a.w) mediaAlbums[ai][8] = [Math.round(a.e / a.w), Math.round(a.v / a.w)];
const mediaOut = `// GENERATED by build-data.js — lazy media index (header search + album pages + Explore depth).
// albums = [title, artistIdx, plays, firstYear, lastYear, yearTail, coverUrl, meta, mood]  (mood [8] = [energy,valence] 0..100, sparse)
// tracks = [title, artistIdx, plays, albumIdx, yearTail, trackNo?]  (trackNo [5] sparse; from Spotify)
