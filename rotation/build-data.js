...
//   window.ROTATION_LIKED_META = { "<artistSlug>~<trackSlug>": [spotifyId, plays, firstYear, bucketCode] }
...
// legend maps bucketCode → {code,label}. Gitignored + apps.json-listed exactly like variant-of.js.
{
  ...
  for (const r of LIKED_SRC) {
    ...
    META[key] = [r.id, plays, firstYear, code, famId, tempoBpm, energy, valence];
  }
  ...
  const _likedOut = "// GENERATED by build-data.js — Liked-songs meta (join of saved tracks × scrobbles), lazy.\n"
    + "// { \"<artistSlug>~<trackSlug>\": [spotifyId, plays, firstYear, bucketCode, familyId, tempoBpm, energy0to100, valence0to100] }\n"
    + "//   familyId/audio are null where unknown. LEGEND = bucketCode→label; FAMS = genre families (i/hue align with the Sound Map); SUBS = artistSlug→primary subgenre.\n"
...
// so the pulse stays meaningful even on a stale rebuild. Shares = fraction of plays in the window.
{
  ...
  const win = (days) => {
    ...
    return { plays, families, topArtists };
  };
...
console.log(`music-core.js written (${(out.length / 1024).toFixed(0)} KB) · music-rest.js written (${(restOut.length / 1024).toFixed(0)} KB)`);
