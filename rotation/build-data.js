...
// ─────────── disc splits (CD1 / CD2) ───────────
// Fuad 2026-09-01: "The Fragile album needs to be properly split into CD1 and 2". Its 23 tracks
// carried per-disc positions — 1..12 then 1..11 — in ONE flat list, so every number 1..11 showed
// twice. 141 albums show 3+ duplicated positions, so this was general, not a NIN quirk.
//
// Neither number source carries a disc: mb-releases has no `thefragile` key at all (and its discs
// array mixes true discs with alternate EDITIONS, so its index is not a disc number either), and
// the Spotify path kept track_number while dropping disc_number. spotify-disc-numbers.json
// (enrich-disc-numbers.js) recovers it from the local dump. See that script's header.
//
// THE GATE is the important part. Raw disc data splits 45 albums, and most of those splits are
// junk: Toxicity would grow a "CD 2" holding the single B-side "Johnny", and SOAD's debut one
// live bonus track. Those numbers are factually right — the track really does sit on disc 2 of
// some reissue — but presenting a one-track CD2 on a single-disc album is a worse lie than the
// duplicate numbering it replaces. Requiring EVERY disc to hold 3+ played tracks leaves 10
// albums, all of them genuine doubles (The Fragile, The Wall, Hardwired, Ghosts I-IV, Music Bank,
// two Motörhead comps, …). Absence of a disc entry means "render as today".
const DISCNO = _readJson("spotify-disc-numbers.json") || {};
const MEDIA_DISCS = (() => {
  const perAlbum = new Map();   // albumIdx → Map(trackSlug → disc)
  for (const r of mediaTracks) {
    const ai = r[3]; if (!(ai >= 0)) continue;
    // Bonus rows ([8]) are lifted out into their own sections before the page groups by disc, so
    // counting them here would claim splits that never render — The Wall owns 7 Fragile-era tracks
    // but 5 are deluxe/variant rows, leaving 2 standard ones that cannot form two discs.
    if (r[8] === 1) continue;
    const d = DISCNO[slug(_mArtists[r[1]]) + "~" + slug(r[0])] || 1;
    if (!perAlbum.has(ai)) perAlbum.set(ai, new Map());
    perAlbum.get(ai).set(slug(r[0]), d);
  }
  const out = {};
  for (const [ai, m] of perAlbum) {
    const counts = new Map();
    for (const d of m.values()) counts.set(d, (counts.get(d) || 0) + 1);
    if (counts.size < 2) continue;                                   // single disc → nothing to say
    if (![...counts.values()].every(c => c >= 3)) continue;           // the gate
    const rec = {};
    for (const [s, d] of m) if (d > 1) rec[s] = d;                    // disc 1 is implied by absence
    out[ai] = rec;
  }
  return out;
})();
console.log(`disc splits: ${Object.keys(MEDIA_DISCS).length} albums render as CD1/CD2 (${Object.keys(DISCNO).length} disc-2+ tracks known)`);
...
// discs = { albumIdx: { trackSlug: discNo } } — ONLY the albums that survive the 3-tracks-per-disc
//   gate above, and within them only the tracks on disc 2+. The album page reads the presence of an
//   albumIdx as "render this tracklist as CD 1 / CD 2 / …"; every other album is untouched.
window.ROTATION_MEDIA = { artists: ${JSON.stringify(_mArtists)}, tracks: ${JSON.stringify(mediaTracks)}, albums: ${JSON.stringify(mediaAlbums)}, discs: ${JSON.stringify(MEDIA_DISCS)} };
`;
