...
// legend maps bucketCode → {code,label}. Gitignored + apps.json-listed exactly like variant-of.js.
{
  ...
  for (const r of LIKED_SRC) {
    const aSlug = slug(canon(r.artist));          // fold artist name before slugging (HAND_MERGE)
    ...
    if (brand) { dist.brand++; META[key] = [r.id, plays, firstYear, brandCode(brand)]; }
    else META[key] = [r.id, plays, firstYear, CODE[bkey]];
  }
  ...
  const _likedOut = "// GENERATED by build-data.js — Liked-songs meta (join of saved tracks × scrobbles), lazy.\n"
    + "// { \"<artistSlug>~<trackSlug>\": [spotifyId, plays, firstYear, bucketCode] } + LEGEND (code→label).\n"
