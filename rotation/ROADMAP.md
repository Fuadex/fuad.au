...
2. **Vocals campaign** — DONE through wave 5 (2026-08-18): research waves with library-track
   disambiguation context → independent verification → merge; vocals.json at 2,204 artists;
   Explore without-data 3,814 → 2,144 (sub-12-play tail + the MB-lineup weekly cron keeps
   nibbling). Owner-verdict lane (highest authority) settles acts Fuad knows firsthand;
   permanently-unclassifiable keys (rotating game-music umbrellas, cover-series, non-act
   uploads) are excluded from future target pools. Wave 6 over the tail optional — same recipe.
