...
function AttrScatter({ rows, mode, xKey, yKey, shade, famDim, go, onBrushSel, pageActiveOf }) {
  ...
  const shadeVal = React.useCallback((row) => {
    ...
    // seen-live flipped 2026-09-13 (Fuad): bright = you HAVE seen them, which is what the option name
    // implies. recency was right as it stood and is untouched — bright = played more recently.
    if (shade === "seenLive") { if (row.seenLive == null) return null; return row.seenLive ? 1 : 0.15; } // brighter = SEEN live
    if (shade === "recency") { if (row.recency == null || !shadeExtent) return null; return (row.recency - shadeExtent.mn) / (shadeExtent.mx - shadeExtent.mn); } // brighter = played more recently
  ...
  return (
    <div>
          ...
          return (
            <button key={f.i} className="xp-attr-legitem" data-on={on} data-dim={famDim != null && !on}
              ...
              {/* Full names on desktop, phone labels below 760px (Fuad 2026-08-21, revising his
                  ...
                  key had room, which on a phone it does not). Both ship and CSS picks one. */}
              <span className="xp-leg-full">{f.family}</span>
              <span className="xp-leg-tiny">{famTiny(f.family)}</span>
            </button>
          );
        })}
      ...
      <div className="r-mono xp-attr-foot">
        ...
        {shade === "seenLive" ? " · brighter = seen live" : shade === "recency" ? " · brighter = played more recently" : ""}
    ...
    <div className="r-view xp" ref={ref}>
      ...
      <style>{`
        ...
        @media (max-width: 760px) {
          ...
          /* the colour key under the chart takes the short genre names */
          .xp-leg-full { display: none; }
          .xp-leg-tiny { display: inline; }
