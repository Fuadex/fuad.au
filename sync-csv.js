...
//
// Usage:  LASTFM_API_KEY=xxx node sync-csv.js   (locally also reads ../Fuad-Soudah/Culture_2/.env)
...
if (!KEY) { try { const m = fs.readFileSync(path.join(__dirname, "../Fuad-Soudah/Culture_2/.env"), "utf8").match(/LASTFM_API_KEY\s*=\s*(.+)/); if (m) KEY = m[1].trim(); } catch (e) {} }
