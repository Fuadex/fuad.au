...
"""
update_omdb.py
Pulls the FULL OMDb payload for every film/TV title into omdb_cache.json (nothing is
dropped from the CACHE — Plot, Genre, Director, Writer, Actors, BoxOffice, etc. all
survive there for future features). The SHIPPED overlay (omdb_data.js) carries only
the fields the app actually renders (SHIP_FIELDS below + one synopsis string) — the
2026-07 audit measured the full payload at 66% dead weight. The app attaches this at
runtime as a non-clobbering nested `item.omdb` so it never overwrites existing
data.js / imports.js fields.
...
Run: python update_omdb.py [--dry-run] [--limit N] [--force] [--emit-only]
--emit-only rebuilds omdb_data.js from the committed cache — no key, no network
(the "this PC" half of the two-PC workflow; use it after changing SHIP_FIELDS).
"""
...
# Everything culture-v2.jsx reads off item.omdb (ratings pills + sort axes, awards line,
# imdb link, poster fallback, Reader synopsis). The CACHE keeps the full payload; only
# this subset ships. If a new feature needs another field: add it here, then
# `python update_omdb.py --emit-only` — no API calls burned.
SHIP_FIELDS = ('imdbID', 'imdbRating', 'imdbVotes', 'Metascore', 'Awards', 'Poster', 'Ratings')


def ship_record(v):
    """Trim one cached payload to what renders. 'N/A' values are dropped outright —
    every read site already treats 'N/A' as absent. Plot ships as ONE string with the
    Reader's own preference order (short IMDb-style first, else the full plot)."""
    out = {k: v[k] for k in SHIP_FIELDS if v.get(k) not in (None, '', 'N/A')}
    ps, p = v.get('PlotShort'), v.get('Plot')
    if ps and ps != 'N/A':
        out['PlotShort'] = ps
    elif p and p != 'N/A':
        out['Plot'] = p
...
def write_overlay(cache):
    """omdb_data.js from the cache (True entries only, trimmed per ship_record)."""
    good = {iid: ship_record(v) for iid, v in cache.items()
            if isinstance(v, dict) and v.get('Response') == 'True'}
    header = ('// omdb_data.js — generated by update_omdb.py. Trimmed OMDb payload per item id\n'
              '// (SHIP_FIELDS + one synopsis; the FULL payload lives in omdb_cache.json).\n'
              '// Do not edit by hand. Re-run: python update_omdb.py [--limit N | --emit-only]\n'
              'window.CULTURE_OMDB = ')
...
def main():
    ...
    if EMIT_ONLY:
        good = write_overlay(cache)
        raw = os.path.getsize(OUT)
        print(f'--emit-only: omdb_data.js rebuilt from cache — {len(good)} payloads, '
              f'{raw / 1048576:.2f} MB. No API calls made.')
        return
