...
process.stdout.write(JSON.stringify({train,score,regionNames:W.CULTURE.REGION_NAMES||{}}));
''' % json.dumps(SD.replace('\\', '/'))

data = json.loads(subprocess.check_output(['node', '-e', NODE]))
train, score = data['train'], data['score']
# mirror of REGION_NAMES in culture-v2.jsx (it lives in the JSX, not data.js — keep in sync)
REGION_NAMES = {
    'pl': 'Polish', 'jp': 'Japanese', 'kr': 'Korean', 'us': 'American', 'uk': 'British',
    'fr': 'French', 'it': 'Italian', 'de': 'German', 'su': 'Soviet', 'ru': 'Russian',
    'au': 'Australian', 'ca': 'Canadian', 'ie': 'Irish', 'il': 'Israeli', 'se': 'Swedish',
    'fi': 'Finnish', 'ch': 'Swiss', 'eu': 'European', 'es': 'Spanish', 'nl': 'Dutch',
    'dk': 'Danish', 'cz': 'Czech', 'hu': 'Hungarian', 'bg': 'Bulgarian', 'hr': 'Croatian',
    'gr': 'Greek', 'br': 'Brazilian', 'hk': 'Hong Kong', 'cn': 'Chinese', 'mx': 'Mexican',
    'tw': 'Taiwanese', 'ar': 'Argentine', 'in': 'Indian', 'lb': 'Lebanese', 'eg': 'Egyptian',
    'is': 'Icelandic', 'th': 'Thai', 'nz': 'New Zealand', 'sg': 'Singaporean', 'ee': 'Estonian',
    'uy': 'Uruguayan', 'ge': 'Georgian',
}
...
# raw per-director stats, only for phrasing the `why` line ("your avg 8.7, n=12")
dir_stats = {}
for r in train:
    if r['director']: dir_stats.setdefault(r['director'], []).append(r['rating'])
dir_stats = {d: (float(np.mean(v)), len(v)) for d, v in dir_stats.items()}
...
for r in score:
    ...
    for j in order:
        ...
        f = FEATS[j]; amt = ('+' if contrib[j] > 0 else '−') + f'{abs(contrib[j]):.1f}'
        if f.startswith('med:'): continue  # real contributor, but "Movies (−0.2)" reads as noise
        if f == 'b:director' and r['director']:
            m, n = dir_stats.get(r['director'], (None, 0))
            why.append(f'director {r["director"]} — your avg {m:.1f} (n={n})' if m is not None
                       else f'director {r["director"]} ({amt})')
        elif f == 'b:region' and r['region']:
            why.append(f'{REGION_NAMES.get(r["region"], r["region"])} cinema ({amt})')
        elif f == 'b:decade' and r['year']:
            why.append(f'the {r["year"]//10*10}s ({amt})')
        elif f.startswith('tag:'):
            why.append(f'tag: {LABEL[f]} ({amt})')
        elif f.startswith('bdg:'):
            why.append(f'badge: {LABEL[f]} ({amt})')
        elif f.startswith(('gen:', 'med:')):
            why.append(f'{LABEL.get(f, f)} ({amt})')
        elif f == 'c:fwAvg':
            why.append(f'crowd score ({amt})')
        elif f == 'c:logvotes':
            why.append(f'popularity ({amt})')
