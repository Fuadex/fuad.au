...
| 2026-08-26/27 | **SERVED-TRUTH PROBE — all 1,228 hires rows** (Fuad: "I don't want to see 3MP filtered as 150MPs; tight and dynamically linked"; workshop `.dtmp/tourqc-pass/extraction/probe_served.py` + `probe_flats.py` + `apply_served.js`) | per-row measurement of what the viewer RECEIVES: iiif info.json (⛔ first pass appended /info.json to urls already ending in it — 223 false failures, fixed+rerun) / zoomify+dzi descriptors / Commons TIF 1920-cap / ranged pixel-header parse; flat img probed separately for iiif rows; round-trip-proof apply | **Fable** | **18 rows fixed:** 5 img-only rows gained `flat` caps (declared master ≫ served render, incl. 425MP-declared/11.8MP-served van Gogh class — pyr-ladder rows correctly exempted, tiles-healthy NGA gigapixel rows correctly untouched). ⭐ **13 BROKEN IIIF POINTERS found+fixed** (2 Dalís, 8 Kupkas, Bonington, O'Sullivan, Open Window): NGA endpoints serve ~900px tiles while the same rows' flat files are 3000–4096px — the viewer preferred the WORSE source; `iiif` dropped, dims set to flat truth. The probe is the standing re-check instrument for this class — rerun after any plate campaign. |
| 2026-08-27 | **seenAt loan questions — RULED by Fuad ("were likely loans"), CLOSED** | — | Fuad | `nicholas-chevalier-the-buffalo-ranges` (aus-performing-arts), `christ-in-the-house-of-his-parents` + `ophelia` (NG London vs Tate Britain), `edward-hopper-house-by-the-railroad` (Met vs MoMA) — all resolved as LOANS per the Artizon precedent; seenAt stands as testimony. Do not re-open. |
...
| 2026-08-30 | Production wave 1 (15 hi-res works: tour+hook+interp+fused+beside, .dtmp/tourwave1-hires) | full §5b cascade under the SEVEN PILOT RULINGS + first FINAL SEAL (whole-batch tic read + factual shield + polish); qc2 mechanical gates, 4 crop rounds/18 box fixes, check-besides + validate-refs | Opus drafts/hooks/fuses/besides, Sonnet interps, Fable seal | seal caught: "Cover the X and…" tic in 8/15 tours (varied to 1), Durand-Ruel 1895 thirty→twenty (contradicted own LOG), Talisman "carved"→painted, Delaunay "news photograph"→press illustration, Boldini companion oil called a "sheet", invented figs in Met Merienda beside, 2 polish deletions; count misreporting again universal (fuse 39-token em-dash class, all 10 beside drafters under-reported); identity-first: 0 wrong-object (pilot had 2); merged 399→414 tours / 817→832 reads / 58 besides |
| 2026-08-30 | Production wave 2 (20 NGA ultra-res works: tour+hook+interp+fused+beside, .dtmp/tourwave2-hires) | §5b cascade + FINAL SEAL first full outing + wave-2 hardening instruments proven (logecho, cross-tour rarity scan, companion grounding) | Opus drafts/hooks/fuses/besides, Sonnet interps, Fable seal | hooks applied ~65 corrections on deep-canon works (Copley sitter map, van Dyck chained-mastiff inversion, Hassam British-ensign order, Titian panel→canvas, Landseer dog, Feast three-hands/Dosso layer); seal caught 5 post-gate errors (fallen-birch LOG echo, 3rd removal-tic + human-yardstick dup, Copley see residue, mahlstick→cane, 3 provenance Info hooks) + 2 false alarms correctly NOT fixed (Bernard title, Maddalena pendant); cross-tour scan then caught 5 phrase collisions the manual read missed; interps 17/20 failed 4-gram reuse first pass (new Sonnet failure mode); merged 414→434 tours / 832→852 reads / 78 besides, refs green |
| 2026-08-30 | Production wave 3 (20 works incl. 3 shipped-beside pair completions, .dtmp/tourwave3-hires) | first full run of the WAVE-2 HARDENING: logecho + cross-tour distinctive-phrase scan + companion grounding + diffuse-dark box rule + banned Info hook classes; FINAL SEAL run with 2 non-drafter seal assistants + 1 bundled verifier | Opus drafts/hooks/fuses/besides, Sonnet interps, Fable seal | hooks applied ~93 corrections (heaviest yet): THREE tours had a central argument factually wrong as drafted — Maddalena scale (canvas is half her mother height, not equal), Homer withheld-gaze (she meets the viewer; orange mass is a carried basket), Pollock narrowing-wedge geometry (both lines lean right, three zones); plus Gauguin sulphur-yellow→lemon-lime (6 places), West calumet/Cohoes/pointing hand, Tiepolo two-NGA-versions pin. Cross-tour scan caught 5 authored-move collisions incl. "most sought-after portraitist" RECURRING from wave 2; logecho caught 1 echo + exposed the artist-name scope gap (fixed with tour: prefix); seal assistants found ~17 contradicts-own-LOG, ~15 factual doubts, ~11 unhedged speculations, ~12 reviewer-speak, 4 rhetorical moves in 20/20 tours — all fixed; verifier DISPROVED 2 doubts (Courbet 1855 cat.2810 correct; Matisse 1,200 francs correct). Interps 18/20 failed the reuse gate while self-certifying clean. Merged 434→454 tours / 852→871 reads (1 augmented, shipped Info preserved byte-identical) / 98 besides; refs green |
| 2026-08-30 | Production wave 4 (20 works, .dtmp/tourwave4-hires) | hardened method with the cross-tour scan moved EARLY (right after first qc2, per the wave-3 lesson); FINAL SEAL with 3 non-drafter assistants (2 tours + 1 short-form layer) + 2 bundled verifiers | Opus drafts/hooks/fuses/besides, Sonnet interps, Fable seal | early scan cleared all 6 three-tour phrase collisions in ONE pass (wave 3 needed 2 agents at seal); you-gate fired first time in production (person-address on seenConfidence=null Seurat); 2 identity alarms raised by drafters both DISPROVED on verification (Marquet is Marquet not Dufy; van Dyck "English Jacobean" is Genoese senatorial dress); ~110 hook corrections incl. Lautrec premise rebuilt (Jeanne Wenz was Frederic Wenz's companion + waitress, not his sister), Tiepolo metamorphosis not confined to hands (leg already a stump), Kensett dark mass is foliage not cloud; SEAL found ~55 items incl. TWO TONE issues raised to Fuad (Gauguin tier-set treated Tahitian womens bodies as pure formal levelling and glossed the colony as "half a world from anyone who might argue"; Tiepolo beside aestheticised two sexual assaults as negative-space) — both fixed; then the FULL open list was cleared on request: 39 tour edits, 2nd verifier settled 9 doubts (6 real: Reynolds Italy-return 1752, "from Kent" was the husbands seat, sky inscription likely 19c, Tiepolo lost STATE not home patronage, van Dyck "10 June 1626" dropped as DBI says 1625, Kensett superlative softened, Wright was in Bath 1775-77; 2 DISPROVED and left), and formulaic convergence broken (besides 20/20 A-vs-B closers to 6; interps 9/20 declarative openings to 3). Merged 454→474 tours / 871→890 reads (19 new + 1 augmented) / 118 besides; refs green |
| 2026-09-03 | Production wave 7 (20 works, .dtmp/tourwave7-hires) — mv 4.1 | mv-4 cascade sharpened without changing its shape: zoom tiles + TILE_MAP.json, accessory-zoom law, stack-wide verification, seal-before-merge, fusion LAYER ALLOCATION | Opus drafts/hooks/fuses, Sonnet interps, Fable seal | Three seals and six repair passes. Partitioning QC BY WORK made 4 agents converge — 8 false sentences + 20 false flags introduced while the batch's dominant tic went UP; fixed by re-partitioning by DEFECT CLASS. The LENS LAYER was never swept in 3 passes and held 12 of the final 26 findings. A read-write race (audit reading tours while lens agents wrote them) produced the last blocking error. **Merged 494→514 tours / 903→917 reads with NO companions** — the merge script asserted their absence as a passing condition, since the spec had moved completeness to "end of wave"; the end of the wave never came and the gap read green until Fuad asked. Companions written and merged later at 5133528: 5 of 20 would have rendered LINKLESS (labelled with the catalogue title while the prose said something else — linkRefs drops a ref whose text is absent or ambiguous, silently, in production). One companion overclaimed on an unseen sheet and was narrowed at 08daaac |
| 2026-09-04 | Production wave 8 (25 works, .dtmp/tourwave8-hires) — mv 4.2, be7904e | Shape unchanged, timing changed: gait forecast moved BEFORE drafting and allocating six argumentative SHAPES (cause/cost/category/addressee/asymmetry/survival); mandatory LENS SWEEP after the stop pass; companions merged in-wave; every review finding re-verified before application | Opus drafts/hooks/fuses/besides/interps, Fable orchestration + gates | **The anti-convergence fix worked and is measured:** wave 7's instruction was NEGATIVE ("get off your own tour"), which is what caused the convergence — writers told to leave the same building pick the nearest empty room. Positive shape allocation took antithesis closes 20/20→0/25, shared 4-grams to 0, with 25 distinct openings AND closings. Only 2 collisions, both works writing the NAME of their assigned shape → allocate the shape, ban its label. **Crop QC:** 177 stops, 5 agents; fix agents caught 5 wrong coordinates + 1 arithmetic error + 3 self-defeating box proposals inside the QC reports themselves. **Lens sweep (new):** 66 findings across 25 tours, only 1 clean — a lens takes a stop's scoped observation and promotes it to a universal; in 7 cases the true wording was already written in a stop of the same tour. **17 proposed wordings had to be rewritten** because they re-introduced the fault they were correcting (2 of one sweep's 3 headline fixes) → a reviewer's diagnosis and its proposed wording have different reliability. **Seal blind spot:** briefed on tour+interp but not the HOOK, it flagged 4 Interpretations for material "found nowhere in the tour" — all 4 sourced 13-17× in their hooks and absent by design because the fusion reserved them. **Stale job file:** 15 shipped Infos were corrected in art-about.js, then a companion author read the pre-correction text from canon.json and reinstated 2 retired claims verbatim. **Shipped Infos audited against the new tours: 3 REPLACE + 12 AMEND + 5 KEEP** (15 of 20 carried a live defect — one was telling another museum's object's story). Merged 514→539 tours / 917→922 reads / 183 companions; 0 orphans, 0 dead refs, 0 broken anchors |
| 2026-09-10 | **CANON DATE DEFECTS CLEARED — 10 rows in `artworks.js`** (wave-10 §B 6–8 + §A 3, plus the seven wave-8 row corrections that had been open since 2026-09-04) | one rule applied to every row: **a museum range becomes its midpoint (rounding down on .5) in `year`, and the museum's own dating is prepended to `note`**; a single "c." year is taken as given. Sources were already in the waves' `hook_<id>.md` files — **zero web calls, nothing re-researched**. Proof by before/after eval of `artworks.js` printing every row whose fields differ | Opus (apply), diff-verified | **10 rows changed, and the diff contained exactly those 10** (plus 10 `artistId`-only edits from a concurrent agent, expected). Years moved: `knud-baade-the-wreck` 1830→1840 (V&A 1830s–1840s), `ludwik-de-laveaux-plac-opery-w-paryzu` 1892→1893 (MNW "ok. 1893"), `pierre-auguste-renoir-luncheon-of-the-boating-party` 1880→1881, `a-young-woman-seated-at-a-virginal` 1670→1671 (NG c. 1670–72), `a-young-girl-reading` 1770→1769 (NGA c. 1769), `derby-portrait-of-a-gentleman` 1770→1771 (c. 1770–1773), `coronation-of-gustav-iii` 1793→1787 (Nationalmuseum 1782–1793). Note-only, year already the midpoint: `titian-the-virgin-suckling-the-infant-christ` (1570), `lawrence-...-3rd-marquess-of-hertford` (1822), `chaim-soutine-untitled` (1923, Orangerie "entre 1923 et 1924", RF 1963 91 — **id deliberately kept**, it keys three other stores). ⚠ **Renoir is the one row where the midpoint was refused:** the Phillips's 1880–81 has midpoint 1880, which is the very value the correction was raised against — the canvas is inscribed 1881 and Durand-Ruel bought it 14 Feb 1881, so the signed year is the value and the span rides in the note. **A midpoint rule that lands back on a known-wrong number is the wrong rule for that row.** ⚠ **Pilo moves furthest (1793→1787) and is the most reversible:** 1793 was the terminal year of a canvas the museum registers as *unfinished*, so the old value asserted a completion date for a picture that has none. 📝 Three record-only findings, no edit possible: `bruno-liljefors-curlew` — **no store we hold carries cm dimensions at all** (checked canon, `art_data`, `art_medium`, `art_imgsize`, `art_hires`), so the museum's transposed 134×112 cannot be "corrected" here; what is recorded instead is that **the landscape plate is right and must never be re-cut to match the catalogue**. `wincenty-kasprzycki-...` — Wikidata's "MNW artwork ID" (P9061) is mis-mapped for this artist, **three of three checked land in the SKAZgr series**; never cite it (working MNW ids: 445858 here, 445548 for the de Laveaux). `unknown-mahakala-in-the-form-of-a-brahman` — the holder fix lives in `art_holders.js`, being regenerated by another agent, so the exact change is written into `.dtmp/tourwave10/DEFECTS.md` §G instead. ⛔ **And it is not the one-line swap it looked like:** `art_holders.places` already resolves the offending `Q63751` to a place named **"Rubin Museum of Art"** at the Rubin's own coordinates, while the Met sits separately at Q160236 with 63 works — so the qid and its generated label disagree, a **second** work (`unknown-bodhisattva-maitreya-the-future-buddha`) carries the same qid, and which institution Q63751 denotes must be settled before anything is swapped |
